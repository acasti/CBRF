function dataA = get_timeA_cosbells3(M, trange, dt, options)
%---------------------------------------------------------------------------------------------------------------
...
%
% USAGE:       dataA = get_timeA_cosbells3(M, trange, dt, options)
...
%              options               * (struct) options structure, which can accept the fields
...
%                     .method          (string)  'exact' or 'interp' (use quicker interpolation)
% OUTPUT:      dataA                 * (struct)  output data struct
% 
% Notes:
...
%    (6) This version attempts to speed up the "exact calculation" using a more efficient search algorithm
%        for the set of cosine bells that overlap any interval [t_{k-1} , t_{k}], which is the major
...
%-------------------------------------------------------------------------------------------------------
function c = cosbell(tvec,t0,width)
%-------------------------------------------------------------------------------------------------------
...
%  do summations over support spikes associated with a fixed spike time "tval"
  c = (1./(2*width)).*(1+cos(pi*(tvec-t0)./width));  
...
%-------------------------------------------------------------------------------------------------------
% Integral of a set of cosine bells between times [a,b] 
% Each cosine bell has a center value 't0' and a 'width'.  These parameters may be vectors, so that
%  the output 'intvals' is also a vector corresponding to each integration of a cosine bell.
...
% This function also performs the mapping from the original time t to the new time A  on the grid.
  ...
  SUPP = zeros(numspikes,6);   % matrix of each spike's support information [i j is0 is1 tspk widthA]
	                             % i = trial#                               (column 1)
                               % j = spike# in trial                      (column 2)
															 % is0 = leftmost support grid index        (column 3)
                               % is1 = rightmost support grid index       (column 4)
															 % tspk = actual spike time                 (column 5)
                               % widthA = cosine bell width for spike     (column 6)
	for i = 1:numtrials
    ...
    for j = 1:numspikes_trial(i)
      ...
      % Here we just keep adding up the cosine bells across all trials, since ultimately we will do a trial
      %  average of the "time A rate functions" anyway in order to obtain the overall rate function for time A.
      indS = is0:is1;   % range of time vector indices which the cosine bell spans for this spike
  ...
  dlamA_mean = dlamA_mean/numtrials;
	...
	% Note that the 'tA' grid will generally not be uniform, even if the original 't' grid was.
...
% Use exact integrations to get transformed "time A" spike times
  ...
  MdlamA = zeros(size(M));
	MA = zeros(size(M));
	[~,numspikes_trial] = spikematrix2vec(M);  % just need number of spikes per trial from this function
  ...
  for nspike = 1:numspikes
    ...
    tspk = spikes_all_sort(nspike);            % The specific spike time under consideration
    ...
    % Get the value of the cosine bell of each support spike (among the set 'tcentervals') evaluated
    %  at the time of the spike under consideration
		c = cosbell(tspk,tcentervals,wvals);
    % Do the same thing for the values of the rate derivative at each spike time
		dc = dcosbell(tspk,tcentervals,wvals);
		MlamA(i,j) = sum(c)/numtrials;      % Just sum all the support-set cosine bells at the spike time
		MdlamA(i,j) = sum(dc)/numtrials;    % Sum all the support-set derivative cosine bells at the spike time
      ...
      [avals,bvals,wvals,tcentervals] = get_support_ranges_for_integral2(tspk_previous,tspk,tsupp_left,tsupp_right,SUPP);
...
%-------------------------------------------------------------------------------------------------------
  % any entries with vsum = 2 constitute the support indices of tspk
...
%-----------------------------------------------------------------------------------------------------------
function [avals,bvals,wvals,tcentervals] = get_support_ranges_for_integral2(tkm1,tk,tsuppL,tsuppR,SUPPSORT)
%-----------------------------------------------------------------------------------------------------------
% New version of algorithm to find all spikes with cosine bell support in the time range [tkm1,tk].
% INPUT:    tkm1                         * left-edge time of interest
%           tk                           * right-edge time of interest
%           tsuppL                       * (vector) t(SUPPSORT(:,3))   (left-most support times)
%           tsuppR                       * (vector) t(SUPPSORT(:,4))   (right-most support times)
%           SUPPSORT                     * matrix of spike time information  [i j is0 is1 tspk widthA] (sorted by tspk)
%                                        * i = trial#                               (column 1)
%                                        * j = spike# in trial                      (column 2)
%                                        * is0 = leftmost support grid index        (column 3)
%                                        * is1 = rightmost support grid index       (column 4)
...
%                                        * widthA = cosine bell width for spike     (column 6)
% OUTPUT:
i1 = find(tsuppR >= tkm1);      % Find right-edge support times >= tkm1
SUPPSORT_1 = SUPPSORT(i1,:);    % Subset of spike data whose right-edge support >= tkm1 
tsuppL_1 = tsuppL(i1);          % (same subset, but for left-edge support time vector)
i2 = find(tsuppL_1 <= tk);      % This gives, among remaining set, spikes whose left-edge support < tk
SUPPSORT_2 = SUPPSORT_1(i2,:);  % Pared-down SUPPSORT matrix of cosine bell data with support in [tkm1,tk]
tcentervals = SUPPSORT_2(:,5);  % spike times with support in [tkm1,tk]
wvals = SUPPSORT_2(:,6);        % cosine bell widths of support spikes
left_edges = tcentervals - wvals;
right_edges = tcentervals + wvals;
avals = tkm1*ones(length(tcentervals),1);  % must be same dimensions (columns) as 'tcentervals' and 'wvals'
bvals = tk*ones(length(tcentervals),1);
% Find bounds of integration appropriate for all the cosine bells
j1 = find(left_edges > tkm1);
avals(j1) = left_edges(j1);
j2 = find(right_edges < tk);
bvals(j2) = right_edges(j2);
save test.mat avals bvals wvals tcentervals tkm1 tk left_edges right_edges;
% LOGIC is currently INCORRECT for finding support sets!  MUST FIX
error('stop!')

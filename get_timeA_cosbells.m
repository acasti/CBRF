function dataA = get_timeA_cosbells(M, trange, dt, randflag, show_progress)
%---------------------------------------------------------------------------------------------------------------
...
%
% USAGE:       dataA = get_timeA_cosbells(M, trange, dt, randflag);
...
%              randflag              * (logical) if true then add a pseudo-random number to the 1/100th msec
%                                       decimal place of each spike time
%              show_progress         * (logical) if true then output progress of calculations to Matlab command window  
...
if (nargin < 4) || isempty(randflag)
	randflag = false;       % do not add random numbers to spike times by default
end
...
%---------------------------------------------------------------------------------------------------------------
% Add a pseudo-random number (PRN) to each spike time if instructed to do so
if randflag
...
[SUPP, lamA_mean, dlamA_mean, tA, numspikes_trial] = get_ratesA_on_grid(t,M,MwidthsA,show_progress);
...
[MlamA,MdlamA,MA] = get_lamA_timeA_each_spike(M,SUPP,show_progress);

% Define some output variables
dataA.t = t;         % original time on the grid
dataA.tA = tA;     % new time A on the grid (same length as t)
...
dataA.randflag = randflag;          % flag indicating whether pseudo random numbers were added to spike times
...
function [SUPP, lamA_mean, dlamA_mean, tA, numspikes_trial] = ...
    get_ratesA_on_grid(t,M,MwidthsA,show_progress)
%-------------------------------------------------------------------------------------------------------
...
%-------------------------------------------------------------------------------------------------------
function [MlamA, MdlamA, MA] = get_lamA_timeA_each_spike(M,SUPP,show_progress)
%-------------------------------------------------------------------------------------------------------
  MlamA = zeros(size(M));
  MdlamA = zeros(size(M));
	MA = zeros(size(M));

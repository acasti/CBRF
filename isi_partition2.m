function data = isi_partition2(M,Mb,Mlam,Mdlam,showplot)
%--------------------------------------------------------------------------------------------------------------------------
...
%
% USAGE:            data = isi_partition2(M,Mb,lam,dlam,showplot);
...
%  (2) Added gamma distribution fit to each of the data subsets
%  (3) Added option to overlay the grand ISI time B distribution over the individual ISIs
%
...
if showplot
  ...
  rescaleY_answer = true;
  if rescaleY_answer
    maxprob_plot = maxprob;
    rescale = true; %#ok<NASGU>
  else  % Give user option to define own y-axis scale maximum
    rescaleY_answer = input('Re-scale the plot axes using your own vertical axis maximum?  (0) No  or  Enter Value : '); %#ok<UNRCH>
    if rescaleY_answer
      rescale = true; 
    ...
    for i =1:numsegx
      for j = 1:numsegy
         ...
         subplot(numsegy,numsegx,k)
           ...
           fsz = 10;
           text(.3*xmax,.9*maxprob_plot,ratestr,'fontweight','bold','fontsize',fsz);
